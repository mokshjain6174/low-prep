...
Low Prep is built around a fast path into the moment that matters: a student asks for help, a tutor accepts, and both enter a dual-pane live room. The left pane is a browser camera/audio call surface for the local demo. The right pane is the Concept Loom, a shared learning workspace that turns the session into structured memory.
...
- Video: browser camera/audio pane for local demo reliability
...
- Swap the local video pane for a production WebRTC provider such as LiveKit, Daily, Twilio, or a self-hosted Jitsi deployment.
